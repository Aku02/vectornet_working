...
class FastBoxRasterizer(Rasterizer):
    ...
    def rasterize(
        ...
        vector_agents_images = []#[None] * len(self.box_history_frames) #
        agents_images = np.zeros(out_shape, dtype=np.uint8)
        vector_ego_images = []#[None] * len(self.box_history_frames)    #
        ...
        for dst_idx, i in enumerate(self.box_history_frames):
            ...
            if agent is None:
                vector_agents_image = [get_agent_coords(agents, raster_from_world), [x["track_id"] for x in agents] + [-1]] 
#                 agents_image = draw_boxes(self.raster_size, raster_from_world, agents, 255)
                vector_ego_image = get_agent_coords(av_agent, raster_from_world)
...
#                         255,
#                     )
                    vector_ego_image = None#np.zeros_like(agents_image)
#                     ego_image = np.zeros_like(agents_image)
                    vector_agents_image = [get_agent_coords(np.append(agents, av_agent), raster_from_world), [x["track_id"] for x in agents] + [-1]]
...
#                         255,
#                     )
                    vector_ego_image = get_agent_coords(agent_ego, raster_from_world) #draw_boxes(self.raster_size, raster_from_world, agent_ego, 255)
#                     ego_image = draw_boxes(self.raster_size, raster_from_world, agent_ego, 255)
            
#             X = []
            
#             print(vector_ego_image.shape)
            
            x = np.zeros((len(vector_agents_image[0]), len(IDX)))
            x[:, :2] = vector_agents_image[0].mean(axis = 1)
            x[:, IDX["other"]] = 1
            x[:, IDX["timestamp"]] = dst_idx
            x[:, IDX["pindex"]] = vector_agents_image[1]
            
            x = x[(x[:,0] >= 0) * (x[:,1] >= 0) * (x[:,0] <= 224*2**8) * (x[:,1] <= 224*2**8)]
            
            vector_agents_images.append(x)
            
            if vector_ego_image is not None:
                x = np.zeros((1, len(IDX)))
                x[:, :2] = vector_ego_image.mean(axis = 1)
                x[:, IDX["ego"]] = 1
                x[:, IDX["timestamp"]] = dst_idx
                x[:, IDX["pindex"]] = -2
...
#         out_im = np.concatenate((agents_images, ego_images), -1)
        out_im = None#   np.concatenate((agents_images, ego_images), -1)
        X = np.concatenate(vector_agents_images, axis = 0)
#         print(X.shape, X[(X[:,0] >= 0) * (X[:,0] <= 224*2**8) * (X[:,1] >= 0) * (X[:,1] <= 224*2**8)].shape)
#         X = X[(X[:,0] >= 0) * (X[:,0] <= 224*2**8)]
#         X = X[(X[:,1] >= 0) * (X[:,1] <= 224*2**8)]
        return {'image_box': out_im, "ego": ego_images, "other": X}
...
#         X = np.array(X)
#         print(X.shape, X[(X[:,0] >= -0.1 * 224*2**8) * (X[:,0] <= 224*2**8 * 1.1) * (X[:,1] >= -0.1 * 224*2**8) * (X[:,1] <= 224*2**8 * 1.1)].shape)
        return {
            'image_semantic': img,
            'tl_lanes_masks4': non_missing_rasterized_tl_lanes,
            "default": lanes_lines["default"],
            "green": lanes_lines["green"],
            "yellow": lanes_lines["yellow"],
            "red": lanes_lines["red"],
            "crosswalks": crosswalks,
            "X": np.array(X), #[(X[:,0] >= 0) * (X[:,0] <= 224*2**8) * (X[:,1] >= 0) * (X[:,1] <= 224*2**8)],
...
#         print(type(im_out_sat), im_out_sat.keys())
        return {
            **im_out_sat,
            **im_out_box,
...
#             "RGB": self.to_rgb(np.concatenate([im_out_box["image_box"], im_out_sat["image_semantic"]], -1))
        }
